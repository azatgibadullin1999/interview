...
static int	is_valid(char symbol) {
	return symbol == CHAR_ALIVE || symbol == CHAR_DEAD ? 1 : 0;
}
...
void initialize_world_from_file(const char * filename) {
	...
	while ((symbol = fgetc(file)) != EOF) {
		...
		if (!is_valid(symbol)) {
			fprintf(stderr, "forbidden charecter, you should to use \'*\' or \' \'\n");
			abort();
		}
		if (i < WORLDWIDTH && j < WORLDHEIGHT) {
			world[i][j] = symbol == CHAR_ALIVE ? ALIVE : DEAD;
		}
		++i;
	}
}
